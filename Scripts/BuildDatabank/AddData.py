...
for key_mol in lipids_dict:
    ...
    if key_mol in sim['COMPOSITION'].keys():
       m_file = sim['COMPOSITION'][key_mol][1]
       with open('./mapping_files/'+m_file,"r") as f:
           for line in f:
               if len(line.split()) > 2 and "Individual atoms" not in line:
                   ...
                   selection = "resname " + sim['COMPOSITION'][key_mol][0]
                   #print(selection)
                   break
...
for key_mol in lipids_dict:
    ...
    try:              
        sim['COMPOSITION'][key_mol].append([leaflet1, leaflet2]) 
    except KeyError:
        continue
    else:
...
for key_mol in molecules_dict:
    try:
        mol_name = sim['COMPOSITION'][key_mol][0]
    except KeyError:
        continue
    else:
        mol_number = u0.select_atoms("resname " + mol_name).n_residues
        sim['COMPOSITION'][key_mol].append(mol_number)
        print("Number of " + key_mol  + ": " + str(sim['COMPOSITION'][key_mol][1]))   
...
for key_mol in all_molecules:
    try:
        mapping_file = './mapping_files/'+sim['COMPOSITION'][key_mol][1]
    except:
        continue
    ...
    try:
        number_of_atoms += np.sum(sim['COMPOSITION'][key_mol][2]) * mapping_file_length
    except:
        continue
