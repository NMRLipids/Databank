...
# coding: utf-8
"""
`makeRanking.py` creates different types of ranking lists of simulations based on their
quality against experiments. The ranking lists are stored in `Data/Ranking/` folder in
json format. The lists can be shown with the `plotQuality.ipynb`
"""
...
if __name__ == "__main__":
    ...
    for SortBasedOn in Fragments:
        ...
        SortedQualities = sorted(NewQualities,
                                 key=lambda i: i['TotalQuality'][SortBasedOn],
                                 reverse=True)

        outputfile = os.path.join(NMLDB_DATA_PATH, 'Ranking',
                                  'SYSTEM_' + SortBasedOn + '_Ranking.json')
        with open(outputfile, "w") as fp:
            json.dump(SortedQualities, fp, default=str, cls=CompactJSONEncoder)
        print(f"Sorted based on {SortBasedOn} quality and saved to {outputfile}")
    ...
    for SortBasedOn in Fragments:
        for lipid in lipids_dict:
            ...
            if SortedQualities:
                outputfile = os.path.join(NMLDB_DATA_PATH, 'Ranking',
                                          lipid + '_' + SortBasedOn + '_Ranking.json')
                with open(outputfile, "w") as fp:
                    json.dump(SortedQualities, fp, default=str, cls=CompactJSONEncoder)
                print(f"Quality of {SortBasedOn} of {lipid} "
