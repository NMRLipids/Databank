...
def plotSimulation(ID, lipid):
    """
    Creates plots of form factor and C-H bond order parameters for the selected ``lipid`` from a 
    simulation with the given ``ID`` number.
    ...
    try:
        ...
        ffdir = os.path.join(NMLDB_EXP_PATH, 'FormFactors', readme['EXPERIMENT']['FORMFACTOR'])
