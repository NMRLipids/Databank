...
def SQL_Create(Table: str, Values: dict, Condition: dict = {}) -> str:
    ...
    Query = (
        ...
        + ") VALUES ("
        + ", ".join(map(lambda x: f'"{x}"', Values.values()))
        + ") "
    )
...
def CreateEntry(Table: str, Information: dict) -> int:
    ...
    if not ID:
        print(f"WARNING: Something may have gone wrong with the table {Table}")
        print(Information)
        return 0
    # If an ID is obtained, the entry was created succesfuly
    print(f"A new entry was created in {Table}: index {ID}")
    return ID
...
def DBEntry(Table: str, Information: dict, Minimal: dict = {}) -> tuple:
    ...
    if EntryID:
        ...
        print(
            f"It was not necessary to update entry {EntryID} in table {Table}",
        )
        return EntryID
...
if __name__ == "__main__":
    ...
    for expOP in EXP_OP:
        ...
        for file in os.listdir(osp.join(PATH_EXPERIMENTS_OP, expOP)):
            if file.endswith(".json"):
                Info = {"doi": README["DOI"], "path": genRpath(osp.join(PATH_EXPERIMENTS_OP, expOP, file))}
    ...
    for expFF in EXP_FF:
        ...
        for file in os.listdir(osp.join(PATH_EXPERIMENTS_FF, expFF)):
            if file.endswith(".json"):
                Info = {"doi": README["DOI"], "path": genRpath(osp.join(PATH_EXPERIMENTS_FF, expFF, file))}
    ...
    for _README in systems:
        ...
        try:
            ...
            for key in README["COMPOSITION"]:
                if key in NMRDict.lipids_set and key not in HETEROMOLECULES_LIST:
                    ...
                    try:
                        ...
                        Lipid_Quality[key] = {"total": 0, "headgroup": 0, "sn-1": 0, "sn-2": 0}
            ...
            for key in README["COMPOSITION"]:
                if key in NMRDict.molecules_set and key != "SOL" and key not in HETEROMOLECULES_LIST:
                    ...
                    for t in ["total", "headgroup", "tail"]:
                        try:
                            Heteromolecules_Quality[key][t] = (
                                Heteromolecules_Quality[key][t] if not np.isnan(Heteromolecules_Quality[key][t]) else 0
                            )
                        except Exception:
            ...
            if "SOL" in README["COMPOSITION"]:
                ...
                Info = {"trajectory_id": Trj_ID, "water_id": Wat_ID, "water_name": WatName, "number": WatNum}

                # The minimal information of the water in the simulation
                Minimal = {"trajectory_id": Trj_ID, "water_id": Wat_ID}
            ...
            for ion in Ions:
                ...
                Info = {"trajectory_id": Trj_ID, "ion_id": Ions[ion][0], "ion_name": ion, "number": Ions[ion][1]}

                # The minimal information that identifies the ion
                Minimal = {"trajectory_id": Trj_ID, "ion_id": Ions[ion][0]}
            ...
            for hetero in Heteromolecules:
                ...
                Minimal = {"trajectory_id": Trj_ID, "molecule_id": Heteromolecules_ID[hetero]}
            ...
            Info = {"trajectory_id": Trj_ID, "membrane_id": Mem_ID, "name": README["SYSTEM"]}
            ...
            try:
                ...
                QUALITY_SYSTEM = {"total": 0, "headgroup": 0, "tails": 0}

            try:
                FFExp = genRpath(osp.join(PATH_EXPERIMENTS_FF, README["EXPERIMENT"]["FORMFACTOR"]))
            except Exception:
                FFExp = ""
            ...
            Info = {
                ...
                "area_per_lipid": APL,
                "area_per_lipid_file": genRpath(osp.join(PATH_SIMULATIONS, README["path"], "apl.json")),
                "form_factor_file": genRpath(osp.join(PATH_SIMULATIONS, README["path"], "FormFactor.json")),
            ...
            for lipid in Lipids:
                ...
                Minimal = {"trajectory_id": Trj_ID, "lipid_id": Lipids_ID[lipid]}
            ...
            for ion in Ions:
                ...
                Info = {"trajectory_id": Trj_ID, "ion_id": Ions[ion][0]}
            ...
            if "SOL" in README["COMPOSITION"]:
                # Collect the information of the water in the simulation
                Info = {"trajectory_id": Trj_ID, "water_id": Wat_ID}
            ...
            for lipid in Lipids:
                ...
                for t in ["total", "headgroup", "sn-1", "sn-2"]:
                    try:
                        Ranking_lipids[lipid][t] = (
                            Ranking_lipids[lipid][t] if not np.isnan(Ranking_lipids[lipid][t]) else 4242
                        )
                    except Exception:
                                    ...
                                    if file.endswith(".json"):
                                        Info = {
                                            ...
                                            "experiment_id": CheckEntry(
                                                ...
                                                {
                                                    "doi": doi,
                                                    "path": genRpath(osp.join(PATH_EXPERIMENTS_OP, path, file)),
                                                },
                                            ),
                                        }
                                    ...
                                    if file.endswith(".json"):
                                        Info = {
                                            ...
                                            "experiment_id": CheckEntry(
                                                "experiments_FF",
                                                {
                                                    "path": genRpath(osp.join(PATH_EXPERIMENTS_FF, path, file)),
                                                },
                                            ),
                                        }
    ...
    try:
        ...
        with database.cursor() as cursor:
            ...
            maxID = int(open(osp.join(PATH_SIMULATIONS, "COUNTER_ID")).readlines()[0])
